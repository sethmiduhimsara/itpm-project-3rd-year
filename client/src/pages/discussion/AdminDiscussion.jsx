    ...
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        <div style={styles.statsRow}>
          ...
          <div style={styles.statBox}>Visible: {posts.filter(p => p.status === 'Visible').length}</div>
          <div style={styles.statBox}>Hidden: {posts.filter(p => p.status === 'Hidden').length}</div>
        </div>
        ...
        {loading ? (
          ...
          filtered.map(post => (
            <div key={post._id} style={{ ...styles.postCard, opacity: post.status === 'Hidden' ? 0.6 : 1 }}>
              <div style={styles.postHeader}>
                ...
                <span
                  style={{
                    ...styles.statusBadge,
                    backgroundColor:
                      post.status === 'Hidden' ? 'rgba(251, 113, 133, 0.12)' : 'rgba(52, 211, 153, 0.12)',
                    color: post.status === 'Hidden' ? 'var(--danger)' : 'var(--success)',
                  }}
                >
                  {post.status}
                </span>
              ...
              <p style={styles.postMeta}>By {post.author} • {new Date(post.createdAt).toLocaleDateString()}</p>
              <div style={styles.actionRow}>
                <button style={styles.hideBtn} onClick={() => handleHide(post._id)}>
...
const styles = {
  ...
  statBox: { backgroundColor: 'rgba(255, 255, 255, 0.04)', color: 'var(--text)', padding: '10px 18px', borderRadius: '12px', fontWeight: '700', fontSize: '14px', border: '1px solid rgba(255, 255, 255, 0.10)' },
  search: { width: '100%', padding: '10px 14px', borderRadius: '10px', border: '1.5px solid var(--panel-border)', marginBottom: '16px', fontSize: '14px', boxSizing: 'border-box', backgroundColor: 'rgba(255, 255, 255, 0.04)', color: 'var(--text)' },
  ...
  statusBadge: { padding: '3px 10px', borderRadius: '999px', fontSize: '11px', fontWeight: '800', border: '1px solid rgba(255, 255, 255, 0.10)', color: 'var(--text)' },
  postTitle: { fontSize: '17px', color: 'var(--text)', marginBottom: '6px' },
  postBody: { color: 'var(--muted)', fontSize: '14px', lineHeight: '1.6', marginBottom: '6px' },
  postMeta: { color: 'var(--muted2)', fontSize: '12px', marginBottom: '12px' },
  actionRow: { display: 'flex', gap: '10px', flexWrap: 'wrap' },
  hideBtn: { backgroundColor: 'rgba(251, 191, 36, 0.12)', color: '#fbbf24', border: '1px solid rgba(251, 191, 36, 0.35)', padding: '7px 14px', borderRadius: '12px', cursor: 'pointer', fontSize: '13px', fontWeight: '800' },
  deleteBtn: { backgroundColor: 'rgba(251, 113, 133, 0.12)', color: 'var(--danger)', border: '1px solid rgba(251, 113, 133, 0.35)', padding: '7px 14px', borderRadius: '12px', cursor: 'pointer', fontSize: '13px', fontWeight: '800' },
